...
def load_idle_frame(path: Path, direction: str = "down") -> pygame.Surface:
    animations = load_character_animations(path)
    frames = animations.get(direction) or animations["down"]
    return frames[1 if len(frames) > 1 else 0]


def _extract_row(
    sheet: pygame.Surface, row: int, frame_width: int, frame_height: int
) -> list[pygame.Surface]:
