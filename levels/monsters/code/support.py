...
def import_folder(*path):
    frames = []
    for folder_path, _sub_folders, image_names in walk(join(*path)):
...
def tmx_importer(*path):
    tmx_dict = {}
    for folder_path, _sub_folders, file_names in walk(join(*path)):
...
def monster_importer(cols, rows, *path):
    monster_dict = {}
    for _folder_path, _sub_folders, image_names in walk(join(*path)):
        for image in image_names:
            image_name = image.split(".")[0]
