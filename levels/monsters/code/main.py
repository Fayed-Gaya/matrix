...
from .paths import AUDIO_DIR, DATA_DIR, GRAPHICS_DIR
...
class Game:
    ...
    def import_assets(self):
        self.tmx_maps = tmx_importer(DATA_DIR, "maps")

        self.overworld_frames = {
            "water": import_folder(GRAPHICS_DIR, "tilesets", "water"),
            "coast": coast_importer(24, 12, GRAPHICS_DIR, "tilesets", "coast"),
            "characters": all_character_import(GRAPHICS_DIR, "characters"),
        }

        self.monster_frames = {
            "icons": import_folder_dict(GRAPHICS_DIR, "icons"),
            "monsters": monster_importer(4, 2, GRAPHICS_DIR, "monsters"),
            "ui": import_folder_dict(GRAPHICS_DIR, "ui"),
            "attacks": attack_importer(GRAPHICS_DIR, "attacks"),
        }
        self.monster_frames["outlines"] = outline_creator(self.monster_frames["monsters"], 4)

        self.fonts = {
            "dialog": pygame.font.Font(join(GRAPHICS_DIR, "fonts", "PixeloidSans.ttf"), 30),
            "regular": pygame.font.Font(join(GRAPHICS_DIR, "fonts", "PixeloidSans.ttf"), 18),
            "small": pygame.font.Font(join(GRAPHICS_DIR, "fonts", "PixeloidSans.ttf"), 14),
            "bold": pygame.font.Font(join(GRAPHICS_DIR, "fonts", "dogicapixelbold.otf"), 20),
        }
        self.bg_frames = import_folder_dict(GRAPHICS_DIR, "backgrounds")
        self.start_animation_frames = import_folder(GRAPHICS_DIR, "other", "star animation")

        self.audio = audio_importer(AUDIO_DIR)
    ...
    def setup(self, tmx_map, player_start_pos):
        ...
        for obj in tmx_map.get_layer_by_name("Collisions"):
            BorderSprite(
                (obj.x, obj.y), pygame.Surface((obj.width, obj.height)), self.collision_sprites
            )
    ...
    def monster_encounter(self):
        sprites = [
            sprite for sprite in self.monster_sprites if sprite.rect.colliderect(self.player.hitbox)
        ]
